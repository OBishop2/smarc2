...
class RescuePointServer(SMARCActionServer):
    ...
    def __init__(
        ...
        self.wp_pub = self._node.create_publisher(PoseStamped, self.get_parameter("setpoint_topic"), 10)
        self._json_ops: GeoActionParsing = GeoActionParsing() #TODO
        self.BASE_FRAME = self.get_parameter("robot_name").value + "/base_link"
        self.ODOM_FRAME = self.get_parameter("robot_name").value + "/odom"

        self._pub_setpoint = self._node.create_publisher(
            PoseStamped, self.get_parameter("setpoint_topic"), 2
        )
    ...
    def declare_parameters(self):
        ...
        typed_param_declare( #TODO: Make goal
            node,
            "vertical_height_offset",
            3.0,
            "Distance (in meters) at which the drone flies while collecting",
        )

        typed_param_declare( #TODO: Make goal
            node,
            "rope_proximity_threshold",
            1.0,
            "Distance (in meters) at which the drone considers the rope close enough to fly straight forward",
        )

        typed_param_declare( #TODO: Make goal
            node,
            "flat_forward_distance",
            4.0,
            "Distance (in meters) that the drone flies when recovering SAM",
        )
        ...
        typed_param_declare( #TODO: Make goal
            ...
            -10.0,
            "Horizontal distance (in meters) that the drone goes to before beginning recovery",
        )
        ...
        typed_param_declare( #TODO: Make goal
            ...
            7.0,
            "Vertical distance (in meters) that the drone goes to before beginning recovery",
        )
        ...
        typed_param_declare( #TODO: Make goal
            ...
            "incline_threshold",
            7.0,
            "Distance (in meters) that the drone ascends along the incline direction during recovery",
        )
    ...
    def execution_callback(self, goal_handle: ServerGoalHandle) -> ActionResult:
        ...
        self.target_index               = self.get_parameter("target_index_offset").value     # index for point the drone moves towards
        ...
        self._node.rescue_timer = self.create_timer(self.get_parameter("dt").value, self.timer_callback) #TODO: Go through all get Params for string and value
        self.get_logger().info('Rescue Point Follower started.')

        status = self.feedback_loop(goal_handle) #TODO
    ...
    def goal_callback(self, goal_request: ActionType.Goal) -> GoalResponse:
        ...
        geopoint_SAM: GeoPoint = self._json_ops.decode(goal_handle.request.goal, ActS.GOAL) #TODO: Make this work with 2 geopoints
        geopoint_buoy: GeoPoint = #TODO: Make this work
        ...
        buoy_pose_utm = convert_latlon_to_utm(geopoing_buoy)

        try:
            self.SAM_pose_odom = self.transform_goal(SAM_pose_utm, self.ODOM_FRAME)
            ...
            self.buoy_pose_odom = self.transform_goal(buoy_pose_utm, self.ODOM_FRAME)
        ...
        if dist >= self.get_parameter("width_goal_threshold").value:
        ...
        if dist >= self.get_parameter("dist_goal_threshold").value:
            err_str = f"Rejecting goal due to violating distance threshold. Criteria: {dist:.1f} >= {self.get_parameter("dist_goal_threshold").value:.1f}"
            self.logger.info(err_str)
            return GoalResponse.REJECT
    ...
    def feedback_loop(self, goal_handle: ServerGoalHandle):
        ...
        while (self.incline_distance  is not None) and not (self.incline_phase and self.incline_distance < self.get_parameter("setpoint_tolerance")): #TODO: Make sure incline dist set to None
    ...
    def compute_distance(self, pose1 : PoseStamped, pose2 : PoseStamped):
        ...
        if(pose1.header.frame_id != pose2.header.frame_id):
            try:
                ...
                return
        dist = np.sqrt((pose1.pose.x - pose2.pose.x) ** 2 + (pose1.pose.y - pose2.pose.y) ** 2 + (pose1.pose.z - pose2.pose.z) ** 2)
    ...
    def timer_callback(self):
        ...
        if self.SAM_pose_odom_array is None or self.quad_pose is None:
        ...
        if self.pickup_traj_start_point is None:
            ...
            target_pos_A = self.SAM_pose_odom_array
            target_pos_B = self.buoy_pose_odom_array
            ...
            start2D = mid3D[:2] + perp * self.get_parameter("horizontal_distance_from_target")
            self.pickup_traj_start_point   = np.array([start2D[0], start2D[1], mid3D[2] + self.get_parameter("horizontal_distance_from_target")]) # starting point of the tau trajectory 
        ...
        if not self.reached_start:
            ...
            if dist <= self.get_parameter("tau_trajectory_starting_threshold").value:
        ...
        if not self.forward_phase and self.step_index < len(self.waypoints):
            ...
            if dist_to_sam <= self.get_parameter("rope_proximity_threshold").value:
                ...
                self.flat_final = current_pos + self.flat_direction * self.get_parameter("flat_forward_distance").value

                self.get_logger().info(
                    f'Within {self.get_parameter("rope_proximity_threshold").value} m of SAM — switching to flat phase, ')
            else:
                ...
                if(next_distance < self.get_parameter("setpoint_tolerance")):
                    self.step_index += 1
                    self.target_index += 1
                    
                elif(target_distance < self.get_parameter("setpoint_tolerance").value):
                    self.step_index += self.get_parameter("target_index_offset").value
                    self.target_index += self.get_parameter("target_index_offset").value
...
def main(args=None):
    try:
        ...
        setpoint = RescuePointServerServer(node, "recover_SAM", action_type)
